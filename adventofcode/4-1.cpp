...
pair<int, int> total_slept(const vector<event>& events) {
  ...
  for (auto nap : naps) {
    ...
    total_slept += range.second - range.first;
  ...
  return make_pair(total_slept, minute);
}
...
pair<int, int> guard_slept_most(const map<int, vector<event>>& guards_events) {
  ...
  for (auto guard_events : guards_events) {
    pair<int, int> slept = total_slept(guard_events.second);
    if (slept.first > max.first) {
      max = slept;
      max_guard_id = guard_events.first;
    }
  }
  return make_pair(max_guard_id, max.second);
}
