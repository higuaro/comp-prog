...
vector<pair<int, int>> bfs(const board_t& maze, const coordinates_t& key_map,
  ...
  //   int keys1 = __builtin_popcount(n1.keys);
  //   int keys2 = __builtin_popcount(n2.keys);
  //   return keys1 == keys2 ? n1.steps < n2.steps : keys1 < keys2;
  // };
  // priority_queue<node_t, vector<node_t>, decltype(cmp)>
  //   q(cmp);
  queue<node_t> q;
...
cerr << "best solution so far: " << best << endl;
      }
      continue;
    }
