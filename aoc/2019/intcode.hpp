...
public:
  intcode(const std::vector<int64_t>& rom,
      std::function<int64_t()> input_handler = []() -> int64_t {
        std::cout << "input> ";
        std::cout.flush();
        int64_t v; std::cin >> v;
        return v;
      },
      std::function<void(int64_t)> output_handler = [](int64_t v) {
        std::cout << v << std::endl;
      }
    ) : rom(rom), mem(rom), input_handler(input_handler), output_handler(output_handler) {
    add_empty_memory();
  }
  ...
  void run() {
    while (ip < mem.size() && !paused && !halted) {
      int64_t inst = mem[ip];
