...
#include <sstream>
using namespace std;
enum operation { inp, add, mul, div_, mod, eql };
...
instruction_t parse_instruction(const string& line) {
  ...
  if (op == "add")
    ...
    return { operation::div_, to_var(var), operand };
  else if (op == "mod")
...
vector<int64_t> evaluate_program(const vector<instruction_t>& program, const vector<int>& input) {
  int input_index = 0;
  vector<int64_t> vars(4, 0); // x, y, z, w;
  for (auto instruction : program) {
    if (instruction.op == operation::inp) {
      cout << " z = " << vars[variable::z] << endl;
      vars[instruction.var] = input[input_index++];
      continue;
    }
    int64_t a = vars[instruction.var];
    int64_t b;
    ...
    switch (instruction.op) {
      ...
      case operation::div_: vars[instruction.var] = a / b; break;
...
int main() {
  ...
  auto digits = to_digits(99999999999999);
  auto limit = to_digits(99999999999998);
  while (digits != limit) {
    auto z = evaluate_program(monad, digits)[variable::z];
    // cout << "z: " << z << endl;
    if (!z) {
